...
//Simulation parameters (adjusted for grid size study):
const string inp_path = "./";
const string out_path = "/share/centaurus_nas/jmg_temp/energy_vs_gridsize/";
...
void calculateMonopoleFieldsAtPoint(long long int global_index, double field_values[8],
    ...
    // First monopole calculations (COPY from your existing code lines ~420-480)
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        cout << "=== ENERGY VS GRID SIZE STUDY ===" << endl;
        cout << "Grid sizes: ";
    ...
    if (rank == 0) {
        string masterPath = out_path + "energy_vs_gridsize_dx_study_seed=" + to_string(seed) + ".csv";
        masterEnergyFile.open(masterPath.c_str());
        masterEnergyFile << "dx,grid_size,total_energy" << endl;
        masterEnergyFile << fixed << setprecision(8);
        cout << "Created output file: " << masterPath << endl;
    }
    ...
    // Loop over dx values
    for (const auto& dx_val : dx_values) {   
        dx = dx_val;
        dy = dx_val;
        dz = dx_val;

        if (rank == 0) {
            cout << "\n=== Processing dx = " << dx << " ===" << endl;
        }
        ...
        for (const auto& grid_size : grid_sizes) {
            ...
            // Calculate halo sizes (same as 2gamma_loop.cpp)
            ...
            for (i = frontHaloSize; i < coreSize + frontHaloSize; i++) {
                    // Calculate field values on-demand for current point and its 6 neighbors
                ...
                if (i_coord < nx - 1) {
                    ...
                    // Copy center fields for boundary
                ...
                if (j_coord < ny - 1) {
                    long long int y_plus_pos = global_pos + nz;
                    calculateMonopoleFieldsAtPoint(y_plus_pos, y_plus_fields, nx, ny, nz, dx, dy, dz,
                                                x1, y1, z1, x2, y2, z2, k_, k_p, gamma_param_1, gamma_param_2);
                } else {
                    // Copy center fields for boundary
                ...
                if (j_coord > 0) {
                    long long int y_minus_pos = global_pos - nz;
                    calculateMonopoleFieldsAtPoint(y_minus_pos, y_minus_fields, nx, ny, nz, dx, dy, dz,
                                                x1, y1, z1, x2, y2, z2, k_, k_p, gamma_param_1, gamma_param_2);
                } else {
                    // Copy center fields for boundary
                ...
                // Calculate kinetic energy using derivatives
                ...
                // Add potential energy terms (same as before)
        ...
        } // End of grid size loop
    } // End of dx loop

    if (rank == 0) {
        masterEnergyFile.close();
        cout << "\nGrid size vs energy study completed!" << endl;
    }

...
const double MH = 0;
const double MHpm_values[] = {1}; // Add as many values as you want
const int n_MHpm = sizeof(MHpm_values)/sizeof(MHpm_values[0]);
...
const string BC = "fixed"; // fixed or Neumann
...
int main(){
	...
	for(int idx_MHpm = 0; idx_MHpm < n_MHpm; ++idx_MHpm) {
		double MHpm = MHpm_values[idx_MHpm];
		...
		// cout << "Lambda: " << lambda << endl;
			...
			oss << std::fixed << std::setprecision(3) << MHpm;
			std::string MHpm_str = oss.str();
			std::replace(MHpm_str.begin(), MHpm_str.end(), '.', '_');


			string valsPath = "/share/centaurus_nas/jmg_temp/fixed_SOR/singleVals_MHpm_" + MHpm_str + ".txt";
			string SOR_FieldsPath = "/share/centaurus_nas/jmg_temp/fixed_SOR/SOR_Fields_MHpm_" + MHpm_str + ".txt";
			string icPath = SOR_FieldsPath;

...
const std::string ic_type = "random";
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        if (size == 1) { std::cout << "Warning: Only one processor being used. This code is not designed for only one processor and may not work." << std::endl; }
        if (chunk < ny * nz) { std::cout << "Warning: Chunk size is less than the minimum halo size (i.e chunk neighbour data). Code currently assumes this is not the case so it probably won't work." << std::endl; }

    }

    // Define variables for the fields
    std::vector<std::vector<double>> fields(nb_fields, std::vector<double>(2 * totSize, 0.0)); // Define and initialize vector of field vectors.
    std::vector<double> fieldx(nb_fields, 0.0), fieldy(nb_fields, 0.0), fieldz(nb_fields, 0.0), fieldt(nb_fields, 0.0), fieldtt(nb_fields, 0.0), localKinEnergy(nb_fields, 0.0); // Needed for calculation of energy. nb_fields components.
    ...
    if (ic_type == "random") {
        ...
        std::mt19937 generator_2(seed + 1);
        std::mt19937 generator_3(seed + 2);
        std::mt19937 generator_4(seed + 3);
        std::mt19937 generator_5(seed + 4);
        std::mt19937 generator_6(seed + 5);
        std::mt19937 generator_7(seed + 6);
        std::mt19937 generator_8(seed + 7);

        std::uniform_real_distribution<double> distribution(-1.0, 1.0); // Uniform distribution for the phase of the strings
    ...
    if (rank == 0) { std::cout << "Initial data loaded/generated in: " << end.tv_sec - start.tv_sec << "s" << std::endl; }
    ...
    for (TimeStep = 0; TimeStep < nt; TimeStep++) {
        ...
        if (TimeStep == 0 and rank == 0) {
            if (calcEnergy and wallDetect) { valsPerLoop << "Energy" << " " << "NDW" << " " << "ADW_Simple" << " " << "ADW_Full" << std::endl; }
            else {
                if (calcEnergy) { valsPerLoop << "Energy" << std::endl; }
                if (wallDetect) { valsPerLoop << "NDW" << " " << "ADW_Simple" << " " << "ADW_Full" << std::endl; }
            }
        }
        ...
        if (rank == 0 and (calcEnergy or wallDetect)) { valsPerLoop << std::endl; }
        ...
        if (makeGif and TimeStep % saveFreq == 0 and TimeStep != 0) {
            ...
            if (rank == 0) {
                // Create files
                std::string TimeStepPath = dir_path + "fields_timestep=" + std::to_string(TimeStep) + outTag + ".txt";
                std::ofstream Gif(TimeStepPath.c_str());
                Gif << "R0" << " " << "R1" << " " << "R2" << " " << "R3" << " " << "R4" << " " << "R5" << " " << "n1" << " " << "n2" << " " << "n3" << std::endl;

                std::vector<std::vector<double>> fieldsOutnt(nb_fields, std::vector<double>(nPos, 0.0));
                double R0nt;
                double R1nt;
                double R2nt;
                double R3nt;
                ...
                for (j = 0; j < nPos; j++) {
                    ...
                    Gif << R0nt << " " << R1nt << " " << R2nt << " " << R3nt << " " << R4nt << " " << R5nt << " " << n1nt << " " << n2nt << " " << n3nt << std::endl;
	...
	// Simulation Progress Output
        if (rank == 0 and TimeStep % countRate == 0) {
            std::cout << "\rTimestep " << TimeStep << " completed.";
        }
    ...
    if (rank == 0) {
        ...
        std::cout << "\rTimestep " << nt << " completed." << std::endl;

        gettimeofday(&end, NULL);

        std::cout << "Time taken: " << end.tv_sec - start.tv_sec << "s" << std::endl;

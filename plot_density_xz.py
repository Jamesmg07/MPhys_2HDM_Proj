...
# Data directories - matching filed_monopole_analysis.py pattern
DATA_DIR = Path("/share/centaurus_nas/jmg_temp/256_energy_density/")
OUTPUT_DIR = Path("/share/centaurus_nas/jmg_temp/256_energy_density/")
...
def find_energy_density_files():
    ...
    # Pattern: energy_density_xzslice_gamma1={gamma1}pi_gamma2={gamma2}pi_nx={nx}_sep={separation}_seed={seed}_monopole.csv
...
def analyze_falloff_behavior(x_line_data, z_line_data, mono_x, mono_z1, mono_z2):
    ...
    if np.sum(valid_region_x) > 5:
        ...
        print(f"    X-direction (perpendicular to dipole, r > 10):")
    ...
    if np.sum(valid_beyond1) > 5:
        ...
        print(f"    Z-direction (beyond monopole 1, z > {mono_z1:.0f}, r > 10):")
        ...
        falloff_data_beyond1 = {
            ...
            'center': mono_z1,
            'fit_region': f'beyond M1 (z > {mono_z1:.0f}), r > 10'
        }
    else:
    ...
    if np.sum(valid_beyond2) > 5:
        ...
        print(f"    Z-direction (beyond monopole 2, z < {mono_z2:.0f}, r > 10):")
        ...
        falloff_data_beyond2 = {
            ...
            'center': mono_z2,
            'fit_region': f'beyond M2 (z < {mono_z2:.0f}), r > 10'
        }
    else:
...
def plot_density_analysis(energy_file):
    """Create XZ slice heatmap and 1D line plots through monopole positions"""
    
    print(f"\nProcessing: {energy_file.name}")
    
    # Extract parameters from filename
    params = extract_params_from_filename(energy_file)
    if not params:
        print(f"  Error: Could not extract parameters from filename")
        return
    ...
    data = read_energy_density(energy_file)
    if data is None or len(data) == 0:
        print(f"  Error: Could not load energy density data")
        return
    
    # Get monopole positions
    monopole1_pos, monopole2_pos = get_monopole_positions(separation)
    ...
    x_coords = np.unique(data['x'].values)
    z_coords = np.unique(data['z'].values)
    
    # Find closest indices to monopole position
    ...
    nx_len, nz_len = len(x_coords), len(z_coords)
    energy_grid = np.zeros((nz_len, nx_len))
    
    for _, row in data.iterrows():
        xi = np.argmin(np.abs(x_coords - row['x']))
        zi = np.argmin(np.abs(z_coords - row['z']))
        energy_grid[zi, xi] = row['energy_density']
    
    # Extract 1D slices through monopole
    x_line_data = data[np.abs(data['z'] - z_slice) < 1e-6].sort_values('x')
    z_line_data = data[np.abs(data['x'] - x_slice) < 1e-6].sort_values('z')
    
    # Analyze falloff behavior
    falloff_x, falloff_beyond1, falloff_beyond2, falloff_data_x, falloff_data_beyond1, falloff_data_beyond2 = analyze_falloff_behavior(
        x_line_data, z_line_data, mono_x, mono_z1, mono_z2)
    
    # Create figure with subplots
    fig = plt.figure(figsize=(15, 5))
    
    # 2D Heatmap (XZ slice) - removed monopole position markers
    ax1 = plt.subplot(131)
    im = ax1.imshow(energy_grid, extent=[x_coords[0], x_coords[-1], 
                                         z_coords[0], z_coords[-1]],
                    origin='lower', aspect='auto', cmap='hot')
    ax1.axvline(x_slice, color='cyan', linestyle='--', alpha=0.5, linewidth=1, label='Line plot positions')
    ax1.axhline(z_slice, color='cyan', linestyle='--', alpha=0.5, linewidth=1)
    ax1.set_xlabel('x')
    ax1.set_ylabel('z')
    ax1.set_title(f'Energy Density XZ Slice (y={mono_y:.2f})\nγ₁={gamma1}π, γ₂={gamma2}π, sep={separation}')
    ax1.legend()
    divider = make_axes_locatable(ax1)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    plt.colorbar(im, cax=cax, label='Energy Density')
    
    # 1D slice along x - WITH FITTED CURVE (perpendicular to dipole axis)
    ax2 = plt.subplot(132)
    
    # Plot actual data
    x_vals = x_line_data['x'].values
    x_energy = x_line_data['energy_density'].values
    ax2.plot(x_vals, x_energy, 'b-', linewidth=2, label='Energy density')
    
    # Plot fitted curve if available (outer region)
    if falloff_data_x is not None:
        # Create fit range in outer region
        x_fit_range = (np.abs(x_vals - mono_x) > 10.0) & (np.abs(x_vals - mono_x) < 50.0)
        x_fit = x_vals[x_fit_range]
        r_fit = np.abs(x_fit - falloff_data_x['center'])
        
        # Power law fit
        E_power_fit = falloff_data_x['power_A'] * (r_fit ** falloff_data_x['power_exp'])
        ax2.plot(x_fit, E_power_fit, 'r--', linewidth=2, alpha=0.5,
                label=f'Fit (r>10): r^{falloff_data_x["power_exp"]:.2f}')
    
    ax2.set_xlabel('x')
    ax2.set_ylabel('Energy Density')
    title_text = f'Energy Density along x (perpendicular)\n(y={mono_y:.2f}, z={z_slice:.2f})'
    if falloff_x != "Unknown":
        title_text += f'\nOuter falloff: {falloff_x}'
    ax2.set_title(title_text)
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    
    # 1D slice along z - WITH FITTED CURVES IN OUTER REGIONS
    ax3 = plt.subplot(133)
    
    # Plot actual data
    z_vals = z_line_data['z'].values
    z_energy = z_line_data['energy_density'].values
    ax3.plot(z_vals, z_energy, 'g-', linewidth=2, label='Energy density')
    
    # Mark monopole positions with vertical lines
    ax3.axvline(mono_z1, color='gray', linestyle=':', alpha=0.3, label=f'M1 (z={mono_z1:.0f})')
    ax3.axvline(mono_z2, color='gray', linestyle=':', alpha=0.3, label=f'M2 (z={mono_z2:.0f})')
    
    # Plot fitted curve beyond monopole 1 if available
    if falloff_data_beyond1 is not None:
        # Only plot in region beyond monopole 1
        z_fit_range = (z_vals > mono_z1) & (np.abs(z_vals - mono_z1) > 10.0) & (np.abs(z_vals - mono_z1) < 50.0)
        z_fit = z_vals[z_fit_range]
        r_fit = np.abs(z_fit - falloff_data_beyond1['center'])
        
        E_power_fit = falloff_data_beyond1['power_A'] * (r_fit ** falloff_data_beyond1['power_exp'])
        ax3.plot(z_fit, E_power_fit, 'r--', linewidth=2, alpha=0.5, 
                label=f'Beyond M1: r^{falloff_data_beyond1["power_exp"]:.2f}')
    
    # Plot fitted curve beyond monopole 2 if available
    if falloff_data_beyond2 is not None:
        # Only plot in region beyond monopole 2
        z_fit_range = (z_vals < mono_z2) & (np.abs(z_vals - mono_z2) > 10.0) & (np.abs(z_vals - mono_z2) < 50.0)
        z_fit = z_vals[z_fit_range]
        r_fit = np.abs(z_fit - falloff_data_beyond2['center'])
        
        E_power_fit = falloff_data_beyond2['power_A'] * (r_fit ** falloff_data_beyond2['power_exp'])
        ax3.plot(z_fit, E_power_fit, 'b--', linewidth=2, alpha=0.5,
                label=f'Beyond M2: r^{falloff_data_beyond2["power_exp"]:.2f}')
    
    ax3.set_xlabel('z')
    ax3.set_ylabel('Energy Density')
    title_text = f'Energy Density along z (parallel to dipole)\n(y={mono_y:.2f}, x={x_slice:.2f})'
    if falloff_beyond1 != "Unknown" or falloff_beyond2 != "Unknown":
        title_text += f'\nOuter falloff: {falloff_beyond1} / {falloff_beyond2}'
    ...
    # Save with matching naming convention in same directory as data
    outTag = f"gamma1={gamma1}pi_gamma2={gamma2}pi_nx={nx}_sep={separation}_seed={seed}_monopole"
    output_file = OUTPUT_DIR / f'energy_density_xz_analysis_{outTag}.png'
...
if __name__ == "__main__":
    print("="*60)
    print("ENERGY DENSITY XZ SLICE ANALYSIS")
    print("="*60)
    ...
    print(f"Simulation parameters: Grid={nx}×{ny}×{nz}")
    ...
    # Find energy density files
    energy_files = find_energy_density_files()
    
    if not energy_files:
        print("ERROR: No energy density files found!")
        exit()
    
    print(f"Found {len(energy_files)} energy density file(s)")
    
    # Process each file
    for i, energy_file in enumerate(energy_files):
        print(f"\n[{i+1}/{len(energy_files)}] Processing file...")
        plot_density_analysis(energy_file)

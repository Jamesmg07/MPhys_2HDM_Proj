...
def detect_gamma_mode(filename):
    """Detect if file uses single gamma or two gammas"""
    if 'gamma1=' in filename and 'gamma2=' in filename:
        return 'two_gamma'
...
def extract_gamma_values(filename):
    """Extract gamma value(s) from filename"""
    mode = detect_gamma_mode(filename)
    
    if mode == 'two_gamma':
        gamma1_match = re.search(r'gamma1=([0-9.]+)', filename)
        gamma2_match = re.search(r'gamma2=([0-9.]+)', filename)
        gamma1 = float(gamma1_match.group(1)) if gamma1_match else 0.5
        gamma2 = float(gamma2_match.group(1)) if gamma2_match else 0.0
        return (gamma1, gamma2), mode
    elif mode == 'single_gamma':
        gamma_match = re.search(r'gamma=([0-9.]+)', filename)
        gamma = float(gamma_match.group(1)) if gamma_match else 0.5
        return gamma, mode
    else:
        return 0.5, mode

def format_gamma_string(gamma_info, mode):
    """Format gamma value(s) as string for titles"""
    if mode == 'two_gamma':
        gamma1, gamma2 = gamma_info
        return f"γ₁={gamma1}π, γ₂={gamma2}π"
    else:
        return f"γ={gamma_info}π"

def format_gamma_filename(gamma_info, mode):
    """Format gamma value(s) for filenames"""
    if mode == 'two_gamma':
        gamma1, gamma2 = gamma_info
        return f"gamma1_{gamma1}pi_gamma2_{gamma2}pi"
    else:
        return f"gamma_{gamma_info}pi"

def find_separation_energy_file():
    """Find the separation vs energy file (handles both single and two-gamma cases)"""
    # Try two-gamma pattern first
    files = list(DATA_DIR.glob("separation_vs_energy_gamma1=*_gamma2=*.csv"))
    
    # If not found, try single gamma pattern
    if not files:
        files = list(DATA_DIR.glob("separation_vs_energy_gamma=*.csv"))
    
    # Last resort: any separation vs energy file
    if not files:
    ...
    if files:
        ...
        mode = detect_gamma_mode(files[0].name)
        gamma_info, _ = extract_gamma_values(files[0].name)
        gamma_str = format_gamma_string(gamma_info, mode)
        print(f"    Detected mode: {mode}, Parameters: {gamma_str}")
...
def find_energy_density_files():
    ...
    # Try two-gamma pattern first
    files = list(DATA_DIR.glob("energy_density_gamma1=*_gamma2=*_monopole.csv"))
    
    # If not found, try single gamma pattern
    if not files:
        files = list(DATA_DIR.glob("energy_density_gamma=*_monopole.csv"))
    
    # Last resort: any energy density file
    if not files:
...
def extract_parameters_from_filename(filename):
    ...
    # Extract gamma (handles both single and two-gamma cases)
    gamma_info, mode = extract_gamma_values(filename)
    ...
    return gamma_info, mode, grid_size, separation, seed_val
...
def plot_energy_vs_separation():
    ...
    gamma_info, mode, grid_size, _, seed_val = extract_parameters_from_filename(sep_energy_file.name)
    gamma_str = format_gamma_string(gamma_info, mode)
    gamma_filename = format_gamma_filename(gamma_info, mode)
    ...
    # Save plot
    filename = f'energy_vs_separation_{gamma_filename}_nx{grid_size}_seed{seed_val}.png'
...
def create_energy_density_slice_plot(data, separation, gamma_info, mode, grid_size, seed_val, slice_type='xz', slice_index=None):
    ...
    plt.title(f'Energy Density ({slice_type.upper()} slice, {slice_info})\n'
              f'Separation = {separation}, {gamma_str}, Grid: {grid_size}³, Seed: {seed_val}',
              fontsize=14)
...
def plot_energy_density_snapshots():
    ...
    for i, file in enumerate(energy_files):
        ...
        # Extract parameters
        gamma_info, mode, grid_size, separation, seed_val = extract_parameters_from_filename(file.name)
        gamma_str = format_gamma_string(gamma_info, mode)
        gamma_filename = format_gamma_filename(gamma_info, mode)
        
        print(f"    Parameters: {gamma_str}, separation={separation}, grid={grid_size}³")
        ...
        for slice_type, slice_idx in zip(slice_types, slice_indices):
            try:
                energy_slice = create_energy_density_slice_plot(
                    data, separation, gamma_info, mode, grid_size, seed_val,
                ...
                if energy_slice is not None:
                    ...
                    filename = (f'energy_density_{slice_type}_slice_sep{separation}_'
                              f'{gamma_filename}_nx{grid_size}_seed{seed_val}.png')
                    save_path = OUTPUT_DIR / filename
...
def create_3d_energy_summary():
    ...
    plt.suptitle(f'Energy Density Analysis Summary - {processed_files} data points\n'
                f'{gamma_str}, Grid: {grid_size}³, Seed: {seed_val}, Vacuum Energy: {vacuum_energy:.6f}', 
                fontsize=16)
    
    # Save summary plot
    filename = f'energy_density_summary_{gamma_filename}_nx{grid_size}_seed{seed_val}.png'
    save_path = OUTPUT_DIR / filename
    save_and_close_plot(save_path, f"  [SUCCESS] Saved: {filename}")

...
const vector<double> gamma_mult_1_values = {0.0}; // Array of gamma_1 values to test
const vector<double> gamma_mult_2_values = {0.0}; // Array of gamma_2 values to test
...
int main(int argc, char** argv) {
    ...
    int total_iterations = separations.size() * gamma_mult_1_values.size() * gamma_mult_2_values.size();
    ...
    if (ic_type == "random") {
                ...
                if (rank == 0) {
                    ...
                    if (save_energy_density) {
                        ...
                        for (j = 1; j < size; j++) {
                            ...
                            // Get the sender's count explicitly to avoid mismatches and handle zeros.
                            int recvCount = 0;
                            MPI_Recv(&recvCount, 1, MPI_INT, j, 98, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                            if (recvCount > 0) {
                                MPI_Recv(&full_energy_density[localCoreStart], recvCount, MPI_DOUBLE, j, 99, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                            }
                        }
                    ...
                    if (save_energy_density) {
                        int sendCount = coreSize;
                        MPI_Send(&sendCount, 1, MPI_INT, 0, 98, MPI_COMM_WORLD);
                        if (sendCount > 0) {
                            MPI_Send(energy_density.data(), sendCount, MPI_DOUBLE, 0, 99, MPI_COMM_WORLD);
                        }
                    }
                }
        ...
        } // End of gamma_mult_2 loop
    } // End of gamma_mult_1 loop

    } // End of else if (ic_type == "monopole") block
    MPI_Barrier(MPI_COMM_WORLD);
    if (rank == 0) { std::cout << "All ranks finished. Finalizing." << std::endl; }

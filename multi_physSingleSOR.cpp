...
const double MH = 0;
const double MHpm_values[] = {0.2, 2}; // Add as many values as you want
...
int main(){
	...
	for(int idx_MHpm = 0; idx_MHpm < n_MHpm; ++idx_MHpm) {
		...
		// cout << "Lambda: " << lambda << endl;
			...
			string MHpm_str = to_string(MHpm);
			MHpm_str.erase(MHpm_str.find_last_not_of('0') + 1, string::npos);
			MHpm_str.erase(MHpm_str.find_last_not_of('.') + 1, string::npos);

			string valsPath = "./SOR/singleVals_MHpm_" + MHpm_str + ".txt";
			string SOR_FieldsPath = "./SOR/SOR_Fields_MHpm_" + MHpm_str + ".txt";
